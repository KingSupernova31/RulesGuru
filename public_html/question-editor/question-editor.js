...
let newSavedAnnouncements = JSON.parse(JSON.stringify(savedAnnouncements));

bindButtonAction(document.getElementById("showInformationEditorButton"), function() {
	document.getElementById("informationEditor").style.display = "block";
	document.getElementById("ownerOptionsGreyout").style.display = "block";

	let options = "";
	for (let i of Object.keys(savedAnnouncements)) {
		options += "<option>" + i.replace(/"/g, "&quot;") + "</option>";
	}
	document.getElementById("informationEditorSelect").innerHTML = options;
	document.getElementById("informationEditorText").value = savedAnnouncements[document.getElementById("informationEditorSelect").value];
});

document.getElementById("informationEditorSelect").addEventListener("change", function() {
	document.getElementById("informationEditorText").value = newSavedAnnouncements[document.getElementById("informationEditorSelect").value];
});

document.getElementById("informationEditorText").addEventListener("change", function() {
	newSavedAnnouncements[document.getElementById("informationEditorSelect").value] = document.getElementById("informationEditorText").value;
});

bindButtonAction(document.getElementById("informationEditorCancelButton"), function() {
	document.getElementById("informationEditor").style.display = "none";
	document.getElementById("ownerOptionsGreyout").style.display = "none";
	newSavedAnnouncements = JSON.parse(JSON.stringify(savedAnnouncements));
});

bindButtonAction(document.getElementById("informationEditorConfirmButton"), function() {
	document.getElementById("informationEditor").style.display = "none";
	document.getElementById("ownerOptionsGreyout").style.display = "none";
	savedAnnouncements = JSON.parse(JSON.stringify(newSavedAnnouncements));
	const updateSavedAnnouncements = function() {
		const httpRequest = new XMLHttpRequest();
		httpRequest.timeout = 10000;
		httpRequest.onabort = updateSavedAnnouncements;
		httpRequest.onerror = updateSavedAnnouncements;
		httpRequest.ontimeout = updateSavedAnnouncements;
		httpRequest.onload = function() {
			if (httpRequest.status === 200) {
				if (httpRequest.response) {
					if (httpRequest.response === "Unauthorized") {
					...
					updateSavedAnnouncements();
				}
			} else {
				updateSavedAnnouncements();
			}
		}
		httpRequest.open("POST", "/updateSavedAnnouncements", true);
		httpRequest.setRequestHeader("Content-Type", "application/json");
		httpRequest.send(JSON.stringify({
			"password": document.getElementById("password").value,
			"data": savedAnnouncements
		}));
	};
	updateSavedAnnouncements();
});
