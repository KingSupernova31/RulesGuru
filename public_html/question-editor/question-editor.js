document
	.getElementById("topBannerRightText")
	.appendChild(document.getElementById("rulesLink"));
document
	.getElementById("topBannerRightText")
	.appendChild(document.getElementById("adminAboutLink"));
	...
	if (currentQuestionData.status) {
		...
		if (currentQuestionData.status === "finished") {
			...
			document.getElementById("status").textContent =
				...
				convertRolesToHumanText(verificationsNeeded);
...
let oldTemplateValue, oldMultiEntryValue;
...
const addCardGenerator = function () {
	...
	image.addEventListener("click", function (event) {
		if (
			confirm(
				`Delete card generator ${Number(this.parentElement.id.slice(13))}?`,
			)
		) {
			...
			let regex = new RegExp(
				`\\[card ${this.parentElement.id.slice(13)}\\]`,
				"g",
			);
			...
			for (
				let i = Number(this.parentElement.id.slice(13));
				document.getElementById(`cardGenerator${i}`);
				i++
			) {
				if (
					document.getElementById(`cardGenerator${i}`).childNodes[6]
						.textContent === "Switch to Template"
				) {
					document
						.getElementById(`cardGenerator${i}`)
						.getElementsByTagName("h4")[0].textContent =
						`Card generator ${i - 1} (List):`;
				} else {
					document
						.getElementById(`cardGenerator${i}`)
						.getElementsByTagName("h4")[0].textContent =
						`Card generator ${i - 1} (Template):`;
				}
				document
					.getElementById(`cardGenerator${i}`)
					.setAttribute("id", `cardGenerator${i - 1}`);
	...
	input.addEventListener("keypress", function (event) {
		if (
			event.keyCode === 13 &&
			allCardNamesLowerCase.includes(this.value.toLowerCase())
		) {
			...
			if (
				!questionObj.cardLists[this.parentElement.id.slice(13) - 1].includes(
					this.value,
				)
			) {
				...
				newCard.addEventListener("click", function (event) {
					questionObj.cardLists[
						this.parentElement.parentElement.id.slice(13) - 1
					].splice(
						questionObj.cardLists[
							this.parentElement.parentElement.id.slice(13) - 1
						].indexOf(this.textContent),
						1,
					);
				...
				questionObj.cardLists[this.parentElement.id.slice(13) - 1].push(
					this.value,
				);
				this.value = "";
	...
	multiEntryButton.addEventListener("click", function (event) {
		...
		document.getElementById("multiEntryText").value =
			questionObj.cardLists[this.parentElement.id.slice(13) - 1].join("\n");
		document.querySelector("#multiEntryDiv h3").textContent =
			`Edit list ${Number(this.parentElement.id.slice(13))}`;
		document.querySelector("#multiEntryDiv p").textContent =
			`Add or remove cards from card list ${Number(this.parentElement.id.slice(13))}. Each card must be on its own line, and spelled correctly.`;
	...
	templateButton.addEventListener("click", function (event) {
		...
		document.querySelector("#templateBoxHeading").textContent =
			`Edit template ${templateNum + 1}`;
		document.getElementById("addORGroupButton").textContent = "Add OR group";
		for (let i in questionObj.cardTemplates[templateNum]) {
			if (
				typeof questionObj.cardTemplates[templateNum][i].preset === "number"
			) {
				addPresetTemplateRule(questionObj.cardTemplates[templateNum][i].preset);
			} else {
				addTemplateRule(
					questionObj.cardTemplates[templateNum][i].field,
					questionObj.cardTemplates[templateNum][i].operator,
					questionObj.cardTemplates[templateNum][i].value,
					questionObj.cardTemplates[templateNum][i].fieldOption,
		...
		if (previewWindow) {
			...
			previewWindow.parentData.currentGenerator = templateConvert(
				validatedTemplate.templateWithValidRulesOnly,
				allCards,
				presetTemplates,
			);
			previewWindow.parentData.currentGeneratorErrors =
				validatedTemplate.templateErrors;
		}
	});
...
for (let i in allSubtypes) {
	subtypeOptions +=
		'<option value="' + allSubtypes[i].replace(/"/g, "&quot;") + '" />';
}
...
for (let i in allKeywords.keywordAbilities) {
	keywordOptions +=
		'<option value="' +
		allKeywords.keywordAbilities[i].replace(/"/g, "&quot;") +
		'" />';
}
...
const createTemplate = function () {
	...
	for (let i = 0; i < rules.length; i++) {
		let rule;
		if (rules[i].childNodes.length === 2) {
			//Presets
			rule = {
				preset: presetTemplates.filter(
					(preset) => preset.description === rules[i].childNodes[1].textContent,
				)[0].id,
				orGroup: isNaN(Number(rules[i].dataset.orgroup))
					? null
					: Number(rules[i].dataset.orgroup),
			};
		} else if (rules[i].childNodes.length === 4) {
			//Normal rules
			rule = {
...
const validateTemplate = function (inputTemplate) {
	...
	for (let i in template) {
		...
		//Blank fields
		if (
			!["Toughness", "Power", "Loyalty", "Defense", "Mana cost"].includes(
				template[i].field,
			)
		) {
			if (template[i].value === "") {
		...
		if (template[i].field === "Mana cost") {
			const allowedSymbols = Object.values(symbolMap)
				.filter((symbol) => symbol.type === "mana")
				.map((symbol) => symbol.symbol)
				.concat(Object.keys(pseudoSymbolMap));
			...
			outerloop: while (workingSymbolString.length > 0) {
				...
				templateErrors.push(
					`Cannot parse "${workingSymbolString}" as one or more valid mana symbols.`,
				);
				thisRuleErrored = true;
				break;
			}
		...
		if (template[i].field === "Rules text") {
			if (
				template[i].operator === "Matches" ||
				template[i].operator === "Does not match"
			) {
				try {
					new RegExp(template[i].value);
				} catch (error) {
					templateErrors.push(
						`"${template[i].value}" is not a valid regular expression.`,
					);
					thisRuleErrored = true;
				}
			}
		...
		if (
			["Toughness", "Power", "Loyalty", "Defense"].includes(template[i].field)
		) {
			if (template[i].operator === ">" || template[i].operator === "<") {
				if (!/^\d+$/.test(template[i].value)) {
					templateErrors.push(
						`"${template[i].value}" is not a valid ${template[i].field[0].toLowerCase() + template[i].field.slice(1)} with operator "${template[i].operator}".`,
					);
					thisRuleErrored = true;
				}
			}
...
const switchModes = function (generatorId) {
	...
	if (generator.childNodes[6].textContent === "Switch to Template") {
		...
		generator.childNodes[7].textContent =
			"(" + generator.childNodes[9].childNodes.length + ")";
		generator.childNodes[1].textContent =
			"Card Generator " + genNum + " (Template):";
		...
		generator.childNodes[7].textContent =
			"(" + generator.childNodes[8].childNodes.length + ")";
		generator.childNodes[1].textContent =
			"Card Generator " + genNum + " (List):";
	}
};

let datalistNum = 0;
const addTemplateRule = function (
	field,
	operator,
	value,
	fieldOption,
	...
	if (orGroup > 4) {
		alert(
			"This question appears to be corrupted, please don't edit it and inform the site owner of the issue. (>5 'or' groups.)",
		);
		return;
	}
	...
	if (fieldOptions.length > 0) {
		...
		if (fieldOption) {
			fieldOptionsSelect.value = fieldOption;
		}
	}
	...
	document
		.getElementById("templateDiv")
		.scrollTo(0, document.getElementById("templateDiv").scrollHeight);
	valueInput.focus();
};
...
const addOrGroup = function () {
	...
	for (let rule of rules) {
		if (rule.dataset.orgroup && rule.dataset.orgroup !== "null") {
			allOrGroupsInUse.push(
				isNaN(Number(rule.dataset.orgroup))
					? null
					: Number(rule.dataset.orgroup),
			);
		}
	}
	if (allOrGroupsInUse.length > 4) {
		alert(
			"You can't have more than 5 'or' groups. If you need more than that, let me know and I'll increase the limit.",
		);
		return;
	}
	for (let i = 0; i < 5; i++) {
...
const processTemplateBox = function () {
	...
	if (validatedTemplate.templateErrors.length === 0) {
		const templateNum =
			Number(
				document.getElementById("templateBoxHeading").textContent.slice(14),
			) - 1;
		closeTemplateBox();
...
const createQuestionObj = function () {
	...
	let finalCardGeneratorArray = JSON.parse(
		JSON.stringify(questionObj.cardLists),
	);
	for (let i = 0; i < finalCardGeneratorArray.length; i++) {
		if (
			document.getElementById(`cardGenerator${i + 1}`).childNodes[6]
				.textContent === "Switch to List"
		) {
...
const getIntersectionOfAdminRolesAndQuestionVerification = function (
	lookingFor,
) {
	const result = [];
	for (let role of ["grammarGuru", "templateGuru", "rulesGuru"]) {
		if (lookingFor === "verified") {
			if (
				currentQuestionData.verification &&
				currentLoggedInAdmin.roles[role] &&
				typeof currentQuestionData.verification[role] === "number"
			) {
				result.push(role);
			}
		} else {
			if (
				currentQuestionData.verification &&
				currentLoggedInAdmin.roles[role] &&
				currentQuestionData.verification[role] === null
			) {
				result.push(role);
			}
		}
...
const updateButtons = function () {
	...
	const applicableVerifiedRoles =
		getIntersectionOfAdminRolesAndQuestionVerification("verified");
	const applicableUnverifiedRoles =
		getIntersectionOfAdminRolesAndQuestionVerification("unverified");
	//Update button
	if (
		questionsAreDifferent(lastKnownServerQuestionState, createQuestionObj()) &&
		currentQuestionData.status === "pending"
	) {
	...
	if (currentQuestionData.status === "pending") {
		...
		if (applicableUnverifiedRoles.length > 0) {
			document.getElementById("changeStatusUpwards").style.display = "block";
			document.getElementById("changeStatusUpwards").textContent =
				"Verify: " + convertRolesToHumanText(applicableUnverifiedRoles);
		} else {
	...
	if (currentQuestionData.status === "pending") {
		...
		if (currentLoggedInAdmin.id === currentQuestionData.verification.editor) {
			...
			document.getElementById("changeStatusDownwards").textContent =
				"Unapprove";
		} else if (applicableVerifiedRoles.length > 0) {
			document.getElementById("changeStatusDownwards").style.display = "block";
			document.getElementById("changeStatusDownwards").textContent =
				"Unverify: " + convertRolesToHumanText(applicableVerifiedRoles);
		} else {
			document.getElementById("changeStatusDownwards").style.display = "none";
		}
	} else if (currentQuestionData.status === "finished") {
	...
	if (
		...
		document.getElementById("changeStatusUpwards").textContent =
			"Update & " + document.getElementById("changeStatusUpwards").textContent;
		document.getElementById("changeStatusDownwards").textContent =
			"Update & " +
			document.getElementById("changeStatusDownwards").textContent;
	} else {
...
const update = function () {
	...
	setTimeout(function () {
		//This forces the cursor to update immediately since it waits for all synchronous code to finish.
		const requestObj = {
		...
		requestObj.questionObj.submitterName = document
			.getElementById("submitterName")
			.value.trim();
		...
		httpRequest.onabort = function () {
			document.getElementById("cursorStyle").innerHTML = "";
			alert(
				"There was an error updating the question. (Request aborted.) Please check your internet connection and try again. If the problem persists, please report the issue.",
			);
		};
		httpRequest.onerror = function () {
			document.getElementById("cursorStyle").innerHTML = "";
			alert(
				"There was an unknown error updating the question. Please check your internet connection and try again. If the problem persists, please report the issue.",
			);
		};
		httpRequest.ontimeout = function () {
			document.getElementById("cursorStyle").innerHTML = "";
			alert(
				"There was an error updating the question. (Request timed out.) Please check your internet connection and try again. If the problem persists, please report the issue.",
			);
		};
			...
			if (httpRequest.status === 200) {
				if (httpRequest.response) {
					...
					if (
						...
						response.message === "That question doesn't exist."
					) {
					} else {
					...
					if (
						response.message ===
						`Question #${requestObj.questionObj.id} updated successfully.`
					) {
					...
					alert(
						"There was an error updating the question. (Server returned no response.) Please check your internet connection and try again. If the problem persists, please report the issue.",
					);
				}
			} else {
				alert(
					`There was an error updating the question. (Server returned status code ${httpRequest.status}.) Please check your internet connection and try again. If the problem persists, please report the issue.`,
				);
			}
		};
...
const submit = function () {
	...
	setTimeout(function () {
		...
		httpRequest.onabort = function () {
			document.getElementById("cursorStyle").innerHTML = "";
			alert(
				"There was an error submitting your question. (Request aborted.) Please check your internet connection and try again. If the problem persists, please report the issue.",
			);
		};
		httpRequest.onerror = function () {
			document.getElementById("cursorStyle").innerHTML = "";
			alert(
				"There was an unknown error submitting your question. Please check your internet connection and try again. If the problem persists, please report the issue.",
			);
		};
		httpRequest.ontimeout = function () {
			document.getElementById("cursorStyle").innerHTML = "";
			alert(
				"There was an error submitting your question. (Request timed out.) Please check your internet connection and try again. If the problem persists, please report the issue.",
			);
		};
		httpRequest.onload = function () {
			document.getElementById("cursorStyle").innerHTML = "";
			if (httpRequest.status === 200) {
				if (httpRequest.response) {
						...
						if (!currentQuestionData.id) {
							if (currentLoggedInAdmin.roles.owner) {
								document.getElementById(
									"questionSubmitterNameFieldOwner",
								).style.display = "block";
							} else {
								document.getElementById(
									"questionSubmitterNameFieldEditor",
								).style.display = "block";
								document.getElementById(
									"questionSubmitterNameFieldEditor",
								).textContent =
									"Submitted by " + currentLoggedInAdmin.name || "";
							}
							document.getElementById("submitterName").value =
								currentLoggedInAdmin.name;
						}
					...
					alert(
						"There was an error submitting your question. (Server returned no response.) Please report this error.",
					);
				}
			} else {
				alert(
					`There was an error submitting your question. (Server returned status code "${httpRequest.status}".) Please report this error.`,
				);
			}
		};
...
const executeClearFieldsButton = function () {
	if (
		...
		if (
			confirm("Your question has unsaved changes. Clear all fields anyway?")
		) {
			clearFields();
		}
	} else {
...
const clearFields = function () {
	...
	document.getElementById("questionSubmitterNameFieldOwner").style.display =
		"none";
	document.getElementById("questionSubmitterNameFieldEditor").style.display =
		"none";
	document.getElementById("submitterName").value = "";
...
const getUnfinishedQuestion = function () {
	if (
		...
		if (
			!confirm("Your question has unsaved changes. Load new question anyway?")
		) {
			return;
		}
	}
...
const populateFields = function (question) {
		...
		if (typeof question.cardGenerators[i][0] === "string") {
			...
			for (let j in question.cardGenerators[i]) {
				if (typeof question.cardGenerators[i][j].preset === "number") {
					...
					addTemplateRule(
						question.cardGenerators[i][j].field,
						question.cardGenerators[i][j].operator,
						question.cardGenerators[i][j].value,
						question.cardGenerators[i][j].fieldOption,
						question.cardGenerators[i][j].orGroup,
					);
				}
			}
			processTemplateBox();
	...
	if (currentLoggedInAdmin.roles.owner) {
		...
		if (question.id && question.submitterName) {
			...
			document.getElementById("questionSubmitterNameFieldEditor").textContent =
				"Submitted by " + question.submitterName || "";
		}
	}
	...
	for (let i in savedCardLists) {
		if (typeof savedCardLists[i][0] === "object") {
			savedCardLists[i] = templateConvert(
				savedCardLists[i],
				allCards,
				presetTemplates,
			);
...
document
	.getElementById("getQuestionIdInput")
...
const processMultiEntry = function () {
	...
	for (let i = 0; i < cardList.length; i++) {
		if (!allCardNamesLowerCase.includes(cardList[i].toLowerCase())) {
			if (
				confirm(`${cardList[i]} is not a valid card. Delete it and continue?`)
			) {
				cardList.splice(i, 1);
				i--;
			} else {
	...
	let affectedList = Number(
		document.getElementById("multiEntryHeading").textContent.slice(10),
	);
	...
	document.querySelector(
		`#${document.getElementById(`cardGenerator${affectedList}`).id} ul`,
	).innerHTML = "";
	document.querySelector(
		`#${document.getElementById(`cardGenerator${affectedList}`).id}`,
	).childNodes[7].textContent = "(0)";
...
const convertTypingRealTime = function (element) {
	...
	newValue = newValue.replace(/ {2,}/g, function (match, offset) {
		if (offset < cursorPos && cursorPos < offset + match.length) {
			//Do nothing if the cursor is within the spaces.
			return match;
		} else {
			if (offset < cursorPos) {
	...
	let stringsToExpresify = [
		...
		"+4",
		"+5",
	]; //Also rule citations, and card names
	const stringsToSymbolfy = [
		"W",
		"U",
		"B",
		"R",
	...
	const allCardNamesMinusWords = JSON.parse(JSON.stringify(allCardNames))
		.filter((name) => !cardNamesToIgnore.includes(name))
		.sort(function (a, b) {
	...
	const expressionRegex = new RegExp(
		`(^|\\s|\\(|"|')(` +
			stringsToExpresify.join("|").replace(/[+()]/g, "\\$&") +
			`)($|\\s|[.,)?;'"s])`,
		"gi",
	);
	newValue = newValue.replace(
		expressionRegex,
		function (match, capt1, capt2, capt3, offset) {
			if (
				offset + capt1.length <= cursorPos &&
				cursorPos <= offset + capt1.length + capt2.length
			) {
				//Do nothing if the cursor is within the symbol or the adjacent characters.
				return match;
			} else {
				if (offset < cursorPos) {
					cursorPos += 2;
	...
	const symbolRegex = new RegExp(
		"(^|\\s)(" +
			stringsToSymbolfy.join("|").replace(/\+/g, "\\+") +
			")($|\\s|[.,;])",
		"gi",
	);
	newValue = newValue.replace(
	...
	newValue = newValue.replace(/\[.+?\]/g, function (match, offset) {
		...
		if (/card \d+/.test(cardName)) {
			...
			if (!assignedCards[cardName]) {
				let foundCardlistIndex = questionObj.cardLists.findIndex(
					function (element) {
						return JSON.stringify(element) === `["${cardName}"]`;
					},
				);
				if (
					foundCardlistIndex > -1 &&
					document.querySelector(
						`#cardGenerator${foundCardlistIndex + 1} .modeSwitchButton`,
					).textContent === "Switch to Template"
				) {
					assignedCards[cardName] = `[card ${
						questionObj.cardLists.findIndex(function (element) {
					...
					const newListInput = document.querySelector(
						`#cardGenerator${cardGeneratorNum - 1} input`,
					);
					newListInput.value = cardName;
					newListInput.dispatchEvent(event);
				}
			}
			if (cursorPos >= offset + match.length) {
				//If the cursor was after the expression, move it by the change distance.
				cursorPos += assignedCards[cardName].length - 2 - cardName.length;
			} else if (offset < cursorPos && cursorPos < offset + match.length) {
				//If the cursor was inside the expression, move it to right afterwards.
	...
	for (let i = 0; i < stringsToSymbolfy.length; i++) {
		newValue = newValue.replace(
			new RegExp("\\{" + stringsToSymbolfy[i] + "\\}", "ig"),
			"{" + stringsToSymbolfy[i] + "}",
		);
	}
...
const displayTemplateCardSet = function (template, num) {
	const convertedTemplate = templateConvert(
		template,
		allCards,
		presetTemplates,
	);
	const affectedSubCardGenerator = document.querySelector(
		`#cardGenerator${num + 1} > .subCardGeneratorTemplate`,
	);
	...
	document.querySelector(`#cardGenerator${num + 1} > .cardCount`).textContent =
		"(" + convertedTemplate.length + ")";
	convertedTemplateStorage[num] = convertedTemplate;
};

let questionsListOpen = false;
bindButtonAction(
	document.getElementById("showQuestionsListButton"),
	function () {
		if (questionsListOpen) {
			...
			document.getElementById("showQuestionsListButton").innerHTML =
				"Show Questions List";
			questionsListOpen = false;
			document.getElementById("hideWhenQuestionsListOpen").style.display =
				"block";
		} else {
...
const getQuestionsList = function (callback, timeout) {
	...
	document
		.getElementById("questionsListDisplay")
		.classList.add("awaitingUpdate");
	let response;
	...
	httpRequest.onload = function () {
		document
			.getElementById("questionsListDisplay")
			.classList.remove("awaitingUpdate");
		if (httpRequest.status === 200) {
			if (httpRequest.response) {
				const response = JSON.parse(httpRequest.response);
...
const displayQuestionsList = function (questionsList) {
	...
	document.getElementById("questionsListCount").textContent =
		questionsList.length + " matching questions:";
};
...
document
	...
	.addEventListener("mouseup", function () {
		if (event.offsetX < this.clientWidth) {
			// Ignore a click on scrollbar
			const lineNum = this.value
...
const login = function () {
	...
	httpRequest.onabort = function () {
		document.getElementById("cursorStyle").innerHTML = "";
		alert(
			"There was an error validating your login. (Request aborted.) Please check your internet connection and try again. If the problem persists, please report the issue.",
		);
	};
	httpRequest.onerror = function () {
		document.getElementById("cursorStyle").innerHTML = "";
		alert(
			"There was an unknown error validating your login. Please check your internet connection and try again. If the problem persists, please report the issue.",
		);
	};
	httpRequest.ontimeout = function () {
		document.getElementById("cursorStyle").innerHTML = "";
		alert(
			"There was an error validating your login. (Request timed out.) Please check your internet connection and try again. If the problem persists, please report the issue.",
		);
	};
	httpRequest.onload = function () {
		document.getElementById("cursorStyle").innerHTML = "";
		if (httpRequest.status === 200) {
			if (httpRequest.response) {
				...
				alert(
					"There was an error validating your login. (Server returned no response.) Please check your internet connection and try again. If the problem persists, please report the issue.",
				);
			}
		} else {
			alert(
				`There was an error validating your login. (Server returned status code ${httpRequest.status}.) Please check your internet connection and try again. If the problem persists, please report the issue.`,
			);
		}
	};
...
const turnOffQuestionStuffObserver = function () {
	...
	document
		.getElementById("questionStuffDiv")
		.removeEventListener("input", mutationObserverCallback);
	document
		.getElementById("submitterName")
		.removeEventListener("input", mutationObserverCallback);
};
const turnOnQuestionStuffObserver = function () {
	document
		.getElementById("questionStuffDiv")
		.addEventListener("input", mutationObserverCallback);
	document
		.getElementById("submitterName")
		.addEventListener("input", mutationObserverCallback);
	questionStuffObserver.observe(document.getElementById("questionStuffDiv"), {
		attributes: true,
		childList: true,
		subtree: true,
	});
...
const changeStatusUpwards = function () {
	...
	setTimeout(function () {
		...
		if (
			currentQuestionData.status === "awaiting verification" &&
			questionsAreDifferent(lastKnownServerQuestionState, createQuestionObj())
		) {
			const userInput = prompt(
				"Please enter a description of the changes you made to this question. This will be sent to this question's editor so that they can better avoid these sorts of issues next time.",
			);
			if (userInput === null) {
				document.getElementById("cursorStyle").innerHTML = "";
				return;
		...
		httpRequest.onabort = function () {
			document.getElementById("cursorStyle").innerHTML = "";
			alert(
				"There was an error approving the question. (Request aborted.) Please check your internet connection and try again. If the problem persists, please report the issue.",
			);
		};
		httpRequest.onerror = function () {
			document.getElementById("cursorStyle").innerHTML = "";
			alert(
				"There was an unknown error approving the question. Please check your internet connection and try again. If the problem persists, please report the issue.",
			);
		};
		httpRequest.ontimeout = function () {
			document.getElementById("cursorStyle").innerHTML = "";
			alert(
				"There was an error approving the question. (Request timed out.) Please check your internet connection and try again. If the problem persists, please report the issue.",
			);
		};
		httpRequest.onload = function () {
			getQuestionCount();
			...
			if (httpRequest.status === 200) {
				if (httpRequest.response) {
					...
					alert(
						"There was an error approving the question. (Server returned no response.) Please check your internet connection and try again. If the problem persists, please report the issue.",
					);
				}
			} else {
				alert(
					`There was an error approving the question. (Server returned status code ${httpRequest.status}.) Please check your internet connection and try again. If the problem persists, please report the issue.`,
				);
			}
		};
...
const changeStatusDownwards = function () {
	...
	setTimeout(function () {
		...
		httpRequest.onabort = function () {
			document.getElementById("cursorStyle").innerHTML = "";
			alert(
				"There was an error unapproving the question. (Request aborted.) Please check your internet connection and try again. If the problem persists, please report the issue.",
			);
		};
		httpRequest.onerror = function () {
			document.getElementById("cursorStyle").innerHTML = "";
			alert(
				"There was an unknown error unapproving the question. Please check your internet connection and try again. If the problem persists, please report the issue.",
			);
		};
		httpRequest.ontimeout = function () {
			document.getElementById("cursorStyle").innerHTML = "";
			alert(
				"There was an error unapproving the question. (Request timed out.) Please check your internet connection and try again. If the problem persists, please report the issue.",
			);
		};
		httpRequest.onload = function () {
			getQuestionCount();
			...
			if (httpRequest.status === 200) {
				if (httpRequest.response) {
					...
					alert(
						"There was an error unapproving the question. (Server returned no response.) Please check your internet connection and try again. If the problem persists, please report the issue.",
					);
				}
			} else {
				alert(
					`There was an error unapproving the question. (Server returned status code ${httpRequest.status}.) Please check your internet connection and try again. If the problem persists, please report the issue.`,
				);
			}
		};
		httpRequest.open("POST", "/changeQuestionStatus", true);
...
document
	.getElementById("cancelTemplateBox")
...
document
	.getElementById("cancelMultiEntryBox")
	.addEventListener("click", function () {
		if (
			oldMultiEntryValue.trim() !==
			document.getElementById("multiEntryText").value.trim()
		) {
			if (confirm("Cancel changes?")) {
				closeMultiEntry();
			}
...
const moveElementsOnResize = function () {
	if (window.matchMedia("only screen and (max-width: 40rem)").matches) {
		document
			.getElementById("questionStuffDiv")
			.insertBefore(
				document.getElementById("clearFields"),
		...
		document
			...
			.insertBefore(
				document.getElementById(
					"iHaveNoIdeaWhyThisNeedsToBeHereButIfItGoesAwayTheButtonIsGiant",
				),
		...
		document
			.getElementById("questionStuffDiv")
			.insertBefore(
				document.getElementById("cardGenerators"),
				document.getElementById("tags"),
			);
	} else {
		document
		...
		document
			.getElementById("questionStuffDiv")
			.appendChild(document.getElementById("cardGenerators"));
	}
};
...
const validateWithWorker = function () {
	...
	for (let i = 0; i < questionObj.cardLists.length; i++) {
		templateEmptyness.push(
			document.querySelector(`#cardGenerator${i + 1} > .modeSwitchButton`)
				.textContent === "Switch to List" &&
				document.querySelector(
					`#cardGenerator${i + 1} .subCardGeneratorTemplate`,
				).childNodes.length === 0,
		);
	}
...
const openPreview = function () {
	...
	if (
		...
		if (templateErrors.length > 0) {
			previewWindow.parentData.currentGeneratorErrors = validateTemplate(
				previewWindow.parentData.currentGenerator,
			).templateErrors;
		} else {
			previewWindow.parentData.currentGeneratorErrors = [];
		}
	...
	changePreviewAll();
	previewCardChangeObserver.observe(document.getElementById("cardGenerators"), {
		attributes: true,
		childList: true,
		subtree: true,
	});
...
const changePreviewText = function () {
	previewWindow.parentData.questionObj.answer =
		document.getElementById("answer").value;
	previewWindow.parentData.questionObj.question =
		document.getElementById("question").value;
	previewWindow.parentData.questionObj.id =
		document.getElementById("currentID").textContent;
	previewWindow.parentData.questionObj.submitterName = document
		.getElementById("submitterName")
		.value.trim();
	previewWindow.parentData.questionObj.tags = currentTags;
	previewWindow.parentData.updateText = true;
...
const changePreviewAll = function () {
	...
	previewWindow.parentData.questionObj.tags = currentTags;
	previewWindow.parentData.questionObj.level =
		document.getElementById("level").value;
	previewWindow.parentData.questionObj.complexity =
		document.getElementById("complexity").value;
...
document
	...
	.addEventListener("change", updateLevelOrComplexity);
document
	.getElementById("complexity")
	.addEventListener("change", updateLevelOrComplexity);

let previewChangeMade = false;
const previewCardChangeObserver = new MutationObserver(function () {
	previewChangeMade = true;
});
...
const createMultiEntryCardList = function () {
	const allMultiEntryCards = document
		.getElementById("multiEntryText")
		.value.split("\n");
	let validCards = [];
	let invalidCards = [];
	...
	for (let i in allMultiEntryCards) {
		...
		for (let j in allCardNames) {
			if (
				allMultiEntryCards[i].toLowerCase() === allCardNames[j].toLowerCase()
			) {
...
bindButtonAction(document.getElementById("updateCitationsButton"), function () {
	document.getElementById("ownerOptionsGreyout").style.display = "block";
	document.getElementById("updateCitationsDiv").style.display = "block";
	return;
});
...
bindButtonAction(
	document.getElementById("updateCitationsConfirmButton"),
	function () {
		const toAdd = document.getElementById("addCitationsInput").value;
		if (toAdd) {
			if (
				!toAdd.match(threeDigitRegex) &&
				!toAdd.match(threeDigitWithDecimalRegex) &&
				!toAdd.match(threeDigitWithDecimalAndLettersRegex)
			) {
				alert("Invalid citation(s) to add: " + toAdd);
				return;
			}
		}
		const toRemove = document.getElementById("removeCitationsInput").value;
		if (toRemove) {
			if (
				!toRemove.match(threeDigitRegex) &&
				!toRemove.match(threeDigitWithDecimalRegex) &&
				!toRemove.match(threeDigitWithDecimalAndLettersRegex)
			) {
				alert("Invalid citation(s) to remove: " + toRemove);
				return;
			}
		}

		fetch("/updateCitations", {
			method: "POST",
			...
			.then((response) => {
				if (!response.ok) {
					if (response.status === 500) {
						alert("There was a server error updating the citation(s).");
					} else {
			...
			.catch((error) => {
				alert("There was a network error updating the citation(s).");
				console.error(error);
			});
	},
);
...
bindButtonAction(
	document.getElementById("updateCitationsCancelButton"),
	function () {
		document.getElementById("updateCitationsDiv").style.display = "none";
		document.getElementById("ownerOptionsGreyout").style.display = "none";
		return;
	},
);
const switchCitationTabs = function (event) {
	const addCitationsInput = document.getElementById("addCitationsInput");
	const removeCitationsInput = document.getElementById("removeCitationsInput");
	const addCitationsLabel = document.getElementById("addCitationsLabel");
	const addCitationsDiv = document.getElementById("addCitationsDiv");
	const removeCitationsDiv = document.getElementById("removeCitationsDiv");
	const removeCitationsLabel = document.getElementById("removeCitationsLabel");
	const updateCitationsTabs =
		document.getElementsByClassName("updateCitationsTab");
	for (let i = 0; i < tagEditorTabs.length; i++) {
		updateCitationsTabs[i].classList.remove("activeTab");
	}
	...
	if (event.target.id === "updateCitationsTabAdd") {
		removeCitationsDiv.style.display = "none";
		removeCitationsInput.value = "";
		removeCitationsInput.disabled = true;
		addCitationsDiv.style.display = "block";
		addCitationsInput.disabled = false;
		addCitationsLabel.textContent = "Add";
	}
	if (event.target.id === "updateCitationsTabRemove") {
		removeCitationsDiv.style.display = "block";
		removeCitationsInput.disabled = false;
		removeCitationsLabel.textContent = "Remove";
		addCitationsDiv.style.display = "none";
		addCitationsInput.value = "";
		addCitationsInput.disabled = true;
	}
	if (event.target.id === "updateCitationsTabMove") {
		addCitationsDiv.style.display = "block";
		addCitationsInput.disabled = false;
		addCitationsLabel.textContent = "To";
		removeCitationsDiv.style.display = "block";
		removeCitationsInput.disabled = false;
		removeCitationsLabel.textContent = "From";
	}
};
const updateCitationsTabs =
	document.getElementsByClassName("updateCitationsTab");
for (let i = 0; i < updateCitationsTabs.length; i++) {
	bindButtonAction(updateCitationsTabs[i], switchCitationTabs);
}
...
document
	.getElementById("tagEditorAddRemoveSelect")
	.addEventListener("change", function () {
		if (this.value === "Change") {
			document.getElementById("tagEditorAddRemoveIntoDiv").style.display =
				"block";
		} else {
			document.getElementById("tagEditorAddRemoveIntoDiv").style.display =
				"none";
		...
		if (this.value === "Add") {
			document.getElementById("tagEditorAddRemoveCount1").style.display =
				"none";
		} else {
			document.getElementById("tagEditorAddRemoveCount1").style.display =
				"inline";
...
document
	.getElementById("tagEditorSuggestionsSelect")
	.addEventListener("change", function () {
		if (this.value === "Tag") {
			document.getElementById(
		...
		if (this.value === "Ranges") {
			document.getElementById("tagEditorSuggestionsInput").style.display =
				"none";
			document.getElementById(
...
const clearAdminEditorFields = function () {
	...
	const adminEditorRolesCheckboxes = document.querySelectorAll(
		"#adminEditorRolesList input",
	);
...
bindButtonAction(
	document.getElementById("adminEditorCancelButton"),
	function () {
...
const switchTabs = function (event) {
	...
	event.target.classList.add("activeTab");
	document.getElementById(
		"tagEditor" + event.target.id.slice(12),
	).style.display = "block";
	event.target.blur();
};
...
document
	.getElementById("tagEditorAddRemoveInput1")
	.addEventListener("input", function () {
		let count = 0;
		if (tagData[document.getElementById("tagEditorAddRemoveInput1").value]) {
			count =
				tagData[document.getElementById("tagEditorAddRemoveInput1").value]
					.count;
		}
		document.getElementById("tagEditorAddRemoveCount1").textContent =
			"(" + count + ")";
	});

document
	.getElementById("tagEditorAddRemoveInput2")
	.addEventListener("input", function () {
		let count = 0;
		if (tagData[document.getElementById("tagEditorAddRemoveInput2").value]) {
			count =
				tagData[document.getElementById("tagEditorAddRemoveInput2").value]
					.count;
		}
		document.getElementById("tagEditorAddRemoveCount2").textContent =
			"(" + count + ")";
	});
...
const populateAdminNamesDropdown = function (adminData) {
	...
	for (let i in adminData) {
		if (Object.values(adminData[i].roles).includes(true)) {
			options +=
				"<option>" + adminData[i].name.replace(/"/g, "&quot;") + "</option>";
		}
	}
	for (let i in adminData) {
		if (!Object.values(adminData[i].roles).includes(true)) {
...
const populateAdminEditorFields = function (admin) {
	...
	for (let i in newAdminData) {
		if (
			newAdminData[i].name === admin.childNodes[admin.selectedIndex].textContent
		) {
	...
	for (let i = 0; i < adminEditorRolesCheckboxes.length; i++) {
		const adminObjectKey =
			adminEditorRolesCheckboxes[i].id.slice(11, 12).toLowerCase() +
			adminEditorRolesCheckboxes[i].id.slice(12);
		adminEditorRolesCheckboxes[i].checked = currentAdmin.roles[adminObjectKey];
	}
	document.getElementById("adminEditorInputEmailAddress").value =
		currentAdmin.emailAddress;
	document.getElementById("adminEditorInputEmailFrequency").value =
		currentAdmin.reminderEmailFrequency;
	document.getElementById("adminEditorInputEditLogEmails").value =
		currentAdmin.sendSelfEditLogEmails.toString()[0].toUpperCase() +
		currentAdmin.sendSelfEditLogEmails.toString().slice(1);
};
...
const updateAdminData = function () {
	...
	for (let i in newAdminData) {
		if (
			newAdminData[i].name ===
			adminNamesElement.childNodes[adminNamesElement.selectedIndex].textContent
		) {
			currentAdmin = newAdminData[i];
			id = Number(i);
		}
	...
	currentAdmin.password = document.getElementById(
		"adminEditorInputPassword",
	).value;
	const adminEditorRolesCheckboxes = document.querySelectorAll(
		"#adminEditorRolesList input",
	);
	...
	currentAdmin.emailAddress = document.getElementById(
		"adminEditorInputEmailAddress",
	).value;
	currentAdmin.reminderEmailFrequency = document.getElementById(
		"adminEditorInputEmailFrequency",
	).value;
	currentAdmin.sendSelfEditLogEmails =
		document.getElementById("adminEditorInputEditLogEmails").value === "True";
};
...
const performAdminEdit = function () {
	...
	if (owners.length === 0) {
		alert(
			"You've removed yourself as the site owner without specifying a new one. You probably don't want to do that.",
		);
		return;
	}
	if (owners.length > 1) {
		alert(
			`${owners[0]} and ${owners[1]} can't both be owners. What are you, a communist‽`,
		);
		return;
	}
	for (let i in newAdminData) {
		if (
			!Object.values(newAdminData[i].roles).includes(true) &&
			newAdminData[i].reminderEmailFrequency !== "Never"
		) {
			alert(
				`You probably shouldn't be sending emails to ${newAdminData[i].name} if they're no longer an admin.`,
			);
			return;
		}
	}
	...
	function subsets(set, n) {
		if (!Number.isInteger(n) || n < 0 || n > set.size)
			return (function* () {})();
	...
	for (let subset of subsets(new Set(newAdminData), 2)) {
		...
		if (
			subset[0].emailAddress === subset[1].emailAddress &&
			subset[0].emailAddress !== ""
		) {
			alert(
				`Both ${subset[0].name} and ${subset[1].name} have the same email address.`,
			);
			return;
		}
		if (subset[0].password === subset[1].password) {
			alert(
				`Both ${subset[0].name} and ${subset[1].name} have the same password.`,
			);
			return;
		}
	}
...
setInterval(function () {
	if (
		...
		if (document.getElementById("submitterName").value.trim().length > 0) {
			savedQuestionState.submitterName = document
				.getElementById("submitterName")
				.value.trim();
		}
		localStorage.setItem(
			"savedQuestionState",
			JSON.stringify(savedQuestionState),
		);
	} else {
...
const updateAndForceStatus = function (newStatus, newId) {
	...
	setTimeout(function () {
		...
		questionData.submitterName = document
			.getElementById("submitterName")
			.value.trim();
		const requestObj = {
			password: document.getElementById("password").value,
		...
		httpRequest.onabort = function () {
			document.getElementById("cursorStyle").innerHTML = "";
			alert(
				"There was an error setting the question status. (Request aborted.) Please check your internet connection and try again. If the problem persists, please report the issue.",
			);
		};
		httpRequest.onerror = function () {
			document.getElementById("cursorStyle").innerHTML = "";
			alert(
				"There was an unknown error setting the question status. Please check your internet connection and try again. If the problem persists, please report the issue.",
			);
		};
		httpRequest.ontimeout = function () {
			document.getElementById("cursorStyle").innerHTML = "";
			alert(
				"There was an error setting the question status. (Request timed out.) Please check your internet connection and try again. If the problem persists, please report the issue.",
			);
		};
		httpRequest.onload = function () {
			getQuestionCount();
			...
			if (httpRequest.status === 200) {
				if (httpRequest.response) {
					...
					alert(
						"There was an error setting the question status. (Server returned no response.) Please check your internet connection and try again. If the problem persists, please report the issue.",
					);
				}
			} else {
				alert(
					`There was an error setting the question status. (Server returned status code ${httpRequest.status}.) Please check your internet connection and try again. If the problem persists, please report the issue.`,
				);
			}
		};
...
bindButtonAction(document.getElementById("forceUpdateButton"), function () {
	...
	if (document.getElementById("forceStatusNewId").value.trim() === "") {
		...
		const newId = Number(
			document.getElementById("forceStatusNewId").value.trim(),
		);
		...
		if (httpRequest.status === 200) {
			if (httpRequest.response) {
				...
				const descriptionMapping = {
					...
					awaitingVerificationGrammar:
						"questions awaiting grammar verification",
					awaitingVerificationTemplates:
						"questions awaiting template verification",
				...
				const roleStrings = roles.map(
					(role) => questionCount[role] + " " + descriptionMapping[role],
				);

				document.getElementById("count").textContent =
					"There are " + combineStrings(roleStrings) + ".";
			}
		}
	};
...
document
	...
	.addEventListener(
		"change",
		updateEverythingBecauseICantFigureOutMyStupidCode,
	);
...
const addPresetTemplateRule = function (id, ignoreShift) {
	...
	content.textContent = presetTemplates.filter(
		(preset) => preset.id === id,
	)[0].description;
...
const addPresetRulesToTemplate = function () {
	...
	let allOrGroupsInUse = rules
		.map((rule) => rule.dataset.orgroup)
		.filter((orGroup) => !Number.isNaN(Number(orGroup)));
	...
	let presetRules = presetTemplates.filter(
		(presetTemplate) => presetTemplate.description === presetDesc,
	)[0].rules;
	for (let rule of presetRules) {
		if (typeof rule.preset === "number") {
			...
			const orGroup =
				rule.orGroup === null ? null : rule.orGroup + numOrGroupsAlreadyInUse;
			addTemplateRule(
				rule.field,
				rule.operator,
				rule.value,
